...
export default function StripePaymentPage() {
  ...
  const fetchStripeAccounts = async () => {
    try {
      ...
      const response = await apiClient.getStripeAccounts(stripeAccountsPage, 10)
      setStripeAccounts(response.data)
    } catch (error) {
      const apiError = error as ApiError;
      toast.error(apiError.response?.data?.message || "Failed to fetch Stripe accounts")
    } finally {
    ...
    <div className="min-h-[80vh]">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-xl font-bold text-gray-900 mb-2">Stripe Account Management</h1>
            <p className="text-gray-600">Connect and manage your Stripe accounts for payment processing</p>
          </div>
          <Button
        ...
        <div className="overflow-x-auto">
              ...
              {isLoadingStripeAccounts ? (
                ...
                <TableRow>
                  <TableCell colSpan={8} className="h-32 text-center">
                    <div className="flex flex-col items-center justify-center text-gray-500">
                      ...
                      <p className="text-lg font-medium">No Stripe accounts connected</p>
                      <p className="text-sm mb-4">Connect your first Stripe account to start processing payments</p>
                      <Button
                        onClick={() => setShowStripeConnectModal(true)}
                        className="bg-blue-600 hover:bg-blue-700"
                      >
                ...
                stripeAccounts?.data?.map((account: any) => (
                  <TableRow key={account.id} className="hover:bg-gray-50/50">
                    ...
                    <TableCell>
                      <Badge 
                        variant={account.charges_enabled ? "default" : "outline"}
                        className={account.charges_enabled ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}
                      >
                    ...
                    <TableCell>
                      <Badge 
                        variant={account.payouts_enabled ? "default" : "outline"}
                        className={account.payouts_enabled ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}
                      >
          ...
          <div className="flex items-center justify-between p-4 border-t">
            <div className="text-sm text-gray-600">
              Showing {stripeAccounts?.data?.length} of {stripeAccounts?.pagination?.total_count} accounts
            </div>
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
              ...
              <span className="text-sm">
                Page {stripeAccounts?.pagination?.current_page} of {stripeAccounts?.pagination?.total_pages}
              </span>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setStripeAccountsPage(p => Math.min(stripeAccounts?.pagination?.total_pages, p + 1))}
                disabled={stripeAccountsPage === stripeAccounts?.pagination?.total_pages}
              >
